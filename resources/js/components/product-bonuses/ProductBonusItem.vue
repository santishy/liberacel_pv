<template>
    ...
    >
        <td
            ...
            "
        >
            <span class="inline-block w-1/3 md:hidden font-bold">Producto/Categoría</span
            >{{ productBonus.name }}
        </td>
        <td
             @dblclick.prevent ="dblclick"
            ...
            "
        >
            <span class="inline-block w-1/3 md:hidden font-bold">Puntos</span
            >{{ productBonus.points }}
        </td>
    ...
    methods:{
        ...
        async submit(){
            try{
                ...
                const res = await axios.post(`/product-bonuses/${this.productBonus.id}`,this.form);
